  ...
  it('Should handle a request to add a new note for user 1 ', function (done) {
    ...
    request(app)
      ...
      .end(function (err, res) {
        ...
        res.body.text.should.be.equal(
          'This is angular note',
          'Should match added note text value'
        );
        noteId = res.body.id;
        findNote({ userId: USER_ID_1, id: noteId }, (error, note) => {
          if (err) {
            ...
            should.exist(
              note,
              'Returning null as a response, should return inserted note'
            );
  ...
  it('Should handle a request to get all notes of a user 1', function (done) {
    request(app)
      ...
      .end(function (err, res) {
        ...
        res.body[res.body.length - 1].text.should.be.equal(
          'This is angular note',
          'Should match last note text value '
        );
        getNotes({ userId: USER_ID_1 }, (error, notes) => {
          if (err) {
            ...
            should.exist(
              notes,
              'Returning null as a response, should return all notes of user 1'
            );
            notes[notes.length - 1].text.should.be.equal(
              'This is angular note'
            );
            done();
          }
      ...
      .end(function (err, res) {
        ...
        getNotes({ userId: USER_ID_2 }, (error, notes) => {
          if (err) {
            ...
            should.exist(
              notes,
              'Returning null as a response, should return all notes of user 2'
            );
  ...
  it('Should handle a request to get notes of a user who has not created any note', function (done) {
    request(app)
      ...
      .end(function (err, res) {
        ...
        should.exist(
          res.body,
          'Should return blank array of a user who has not created any note'
        );
...
describe('Testing to update a note', function () {
  ...
  it('Should handle a request to update a note by note id', function (done) {
    request(app)
      .put('/api/v1/notes/' + '60532c39bc76d53b107074ef')
      .expect('Content-Type', /json/)
      .send(updateNote)
      .end(function (err, res) {
        should.not.exist(err);
        should.exist(
          res.body,
          'Returning null as a response, should return updated note'
        );
        res.body.text.should.be.equal(
          'This is angular 6 note',
          'Should match updated note text'
        );
